...
func (l *Flexlist) Draw(p *Painter) {
	...
	for i, item := range l.items {
		...
		p.WithStyle(style, func(p *Painter) {
			p.FillRect(x+l.lPad, y, len(item)-1, 1)
			p.DrawText(x+l.lPad, y, item)
		})

		if len(item)+l.lPad+l.rPad > maxW {
			maxW = len(item) + l.lPad + l.rPad
		}
		y = y + 1
		if y == l.Size().Y {
			y = 0
...
func (l *Flexlist) SizeHint() image.Point {
	...
	for _, item := range l.items {
		if w := stringWidth(item)+l.lPad+l.rPad; w > width {
			width = w
		}
	}
...
func (l *Flexlist) moveRight() {
	if l.selected < len(l.items)-l.Size().Y && l.selected >= 0 {
		l.selected = l.selected + l.Size().Y
	} else {
